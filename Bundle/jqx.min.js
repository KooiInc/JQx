...
${c.message}
Rule: ${he(h)}
${r}`),g}}return{sheet:t,removeRules:y,tryParseAtOrNestedRules:S,ruleExists:f,checkParams:x,tryParse:d,consider:b,currentSheetID:n}}function Ft(t,e){return`${t.trim()} ${Object.entries(e).map(([r,n])=>`${r}: { ${At(n)}`)}`}function he(t){let e=(t||"NO RULE").trim().slice(0,50).replace(/\n/g,"\\n").replace(/\s{2,}/g," ");return t.length>e.length?`${e.trim()}...truncated`:e}function At(t){return Object.entries(t).map(([e,r])=>`${e}: ${r.trim()}`).join(`;
