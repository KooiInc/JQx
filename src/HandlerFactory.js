...
export default function() {
  ...
  const addAndStoreListener = (eventType, handler, capture, once, name) => {
    if (once) {
      return addEventListener(eventType, handler, {once});
    }
    ...
    if (!delegateExists) {
      addEventListener(eventType, handler, {capture: capture || getCapture(eventType), once: once || false});
  ...
  return function(spec) { /*NODOC*/
    ...
    const handler = wrapHandlerFunction(selector, callback);
